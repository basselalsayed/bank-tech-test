def client_spec_setup
  ...
  allow(printer_double).to receive(:print_statement).with(account: account_double)
...
def account_spec_setup
  allow(transaction_double).to receive(:new).with(hash_including(type: 'deposit', amount: kind_of(Numeric))).and_return(deposit)
  allow(transaction_double).to receive(:new).with(hash_including(type: 'withdrawal', amount: kind_of(Numeric))).and_return(withdrawal)
