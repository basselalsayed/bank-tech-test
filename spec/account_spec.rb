...
describe Account do
  ...
  describe '#deposit' do
    it 'creates a new instance of deposit' do
      expect(deposit).to receive(:new).with(amount: 1000, balance: 1000, type: 'deposit')
      account.deposit(amount: 1000, transaction: deposit)
    end
  ...
  describe '#withdraw' do
    it 'raises an error when attempting to withdraw more than balance' do
      expect { account.withdraw(amount: 500, transaction: withdrawal) }
    ...
    context 'withdrawal needs initial balance to be greater than 0' do
      before do
        ...
        it 'creates a new instance of withdrawal' do
          expect(withdrawal).to receive(:new).with(amount: 500, balance: 500, type: 'withdrawal')
          account.withdraw(amount: 500, transaction: withdrawal)
        end
